...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    // domain을 생략
    res.cookie('token', token, { httpOnly: true, secure: true, maxAge: 3600000, path: '/' });

    // domain을 설정
    //res.cookie('token', token, { httpOnly: true, secure: true, maxAge: 3600000, domain: 'first.smc2024.kro.kr', path: '/' }); // 1시간 동안 유효
...
app.post('/logout', (req, res) => {
  console.log('로그아웃 요청 받음'); // 요청 확인
  //res.clearCookie('token', { path: '/', domain: '.first.smc2024.kro.kr', secure: true }); // 쿠키 삭제
...
app.get('/auth/validate-token', (req, res) => {
  const token = req.cookies.token; // Get the token from httpOnly cookie
  ...
  jwt.verify(token, SECRET_KEY, (err, decoded) => {
    ...
    // Return both email and id from the decoded token
